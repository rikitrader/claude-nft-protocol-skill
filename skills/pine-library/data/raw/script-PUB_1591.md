---
id: PUB;1591
title: BUY & SELL VOLUME PRESSURE by @XeL_Arjona
author: xel_arjona
type: indicator
tags: []
boosts: 2253
...
# Source Code
```pine
//	* BUY & SELL PRESSURE VOLUME PRESSURE
//    Ver. 1.00.beta1.27.07.2015
//    By Ricardo M Arjona @XeL_Arjona
//	
//		DISCLAIMER:
//
//      The Following indicator/code IS NOT intended to be
//      a formal investment advice or recomendation by the
...
//		FREELEY AND PUBLICLY available on the Web
//		for NON LUCRATIVE ACTIVITIES and must remain as is.
//
//		Pine Script code MOD's and addaptations by @XeL_Arjona 
...
//      + Normalization (Filter) from Karthik Marar's VSA work:
...
//         It's a simple addaptation of the popular "Price
//      Percentage Oscilator" or MACD but taking Buying Pressure
//      against Selling Pressure Averages, so given a Positive 
//      oscilator reading (>0) represents Bullish dominant Trend
//      and a Negative reading (<0) a Bearish dominant Trend.
//      Histogram is the diff between RAW Volume Pressures
//      Convergence/Divergence minus Normalized ones (Signal)
//      which helps as a confirmator.
//
...
//		
//////////////////////////////////////////////////////////////////
study("BUY & SELL VOLUME PRESSURE by @XeL_Arjona", shorttitle="BSP_XeL", precision=0)
...
vmacd = input(false, title="Buy to Sell Convergence/Div OSC:")
...
vpo1 = (( sum(BPVavg,long)-sum(SPVavg,long))/sum(TPVavg,long))*100
vpo2 = (( sum(nbf,long)-sum(nsf,long))/sum(tpf,long))*100 //ema(wma(vmcdt,signal),signal)
vph = nz((vpo1 - vpo2),0)
// Plot Indicator
...
plot(vmacd ? vph:na, color=histC, title="VPH", style=columns, linewidth=3, transp=80)
```

...
es = (close[2] > open[2] and min(open[1], close[1]) > close[2] and open < min(open[1], close[1]) and close < open and close > .5 * open - close)
plotshape(es, title= "Evening Star",offset = -1, color=red, style=shape.arrowdown, text="Evening\nStar")

ms = (close[2] < open[2] and max(open[1], close[1]) < close[2] and open > max(open[1], close[1]) and close > .5 * close - open)
...
aboveTLine = close < close[1] and close < TLine
belowTLine = close > close[1] and close > TLine 


barcolor(ms and belowTLine and k <= 25 and d <= 25 ? yellow : na)  
barcolor(bek and aboveTLine and k >= 75 and d >= 75 ? fuchsia : na)
barcolor(ps and belowTLine and k <= 25 and d <= 25 ? yellow : na)
barcolor(blk and belowTLine and k <= 25 and d <= 25 ? yellow : na)
barcolor(ble and belowTLine ? yellow : na)
barcolor(dcc and aboveTLine ? fuchsia : na)
barcolor(h and belowTLine and k <= 25 and d <= 25 ? yellow : na)
barcolor(ih and belowTLine and k <= 25 and d <= 25 ? yellow : na)
barcolor(ss and aboveTLine and k >= 75 and d >= 75 ? fuchsia : na)
barcolor(es and aboveTLine and k >= 75 and d >= 75 ? fuchsia : na)
barcolor(dj or ih or beh and aboveTLine and k >= 75 and d >= 75 ? fuchsia : na)
barcolor(dj and belowTLine and k <= 25 and d <= 25 ? yellow : na)
```
